...
**Status:** Review complete — security re-verify (HIGH cleared) + code review (APPROVE, no blocking/high) both signed off; merge-ready once the GitHub CI run is confirmed green.
...
- [x] Code reviewed (§11 — `code-reviewer` APPROVE, no blocking/high; L3/L4/L5 fixed, L1/L2/L6 tracked)
...
**Re-verify (2026-08-07):** the `security` agent re-audited the landed fixes against ADR 0004 and confirmed both layers present & correct — **HIGH cleared for merge, S2 closed.** It raised one new LOW (within the accepted residual class): a _transient_ join-rotation failure reset the roster ref to `null`, so the next roster change would downgrade the retry to `ensure` and re-share the old key. **Fixed:** on failure the ref now reverts to the _previous_ roster, so the retry stays a `rotate`; regression test added ("re-attempts a rotate … after a failed join-rotation").

**Routed elsewhere (not security):** no idempotency/dedup on the persist path (a reconnect re-emit stores a duplicate) → `code-reviewer` (see §11 L1); unbounded `page` deep-`skip()` on a growing collection → `performance`.

---

## 11. Code review — holistic pass (2026-08-07)

The `code-reviewer` agent ran the full `eaz-code-review` checklist + CLAUDE.md §7 against the final branch. **Verdict: APPROVE — no blocking, no high findings.** Layering (thin controller, service holds auth/business logic, repository is the only Mongo toucher), naming, error handling (typed `AppError`, the two intentional `.catch`es log rather than swallow), the ADR 0004 enforcement, and test quality all hold. Merge is gated only on confirming the **GitHub CI run** is green (the agent's env had `gh` unauthenticated; local lint/typecheck/tests are green).

| #   | Severity | Finding                                                                                                                                                                                                                                                                             | Status                                                                                                                                                                                                                           |
| --- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| L1  | LOW      | Persist path has no idempotency key → a re-emit of `chat:group:message:new` stores a duplicate row (inflates `total`, wasted storage). Genuinely low today (client emits once; Socket.IO reconnect doesn't replay app-level emits), but latent once an offline send-queue is added. | **Deferred (tracked).** Add a unique `{groupId, senderId, timestamp}` index + idempotent create/upsert; apply to the 1:1 `messages` path too. Same class as the pre-existing 1:1 dedup deferral. Not security (ciphertext-only). |
| L2  | LOW      | Live incoming handlers (`GroupChatScreen`/`ChatScreen`) append without the `senderId+timestamp` dedup the history-merge uses → duplicate bubbles + React key warning if the server ever redelivers a live event. Pre-existing (the group N6 residual), not new here.                | **Deferred (tracked with N6).** Fold the same dedup into the live handlers when N6 is picked up.                                                                                                                                 |
| L3  | LOW      | History-merge effect re-ran the full decrypt on every roster change (`nameByUser` in deps); harmless (idempotent, first page only) but redundant — a dedicated effect already refreshes names.                                                                                      | **Fixed** — dropped `nameByUser` from the merge effect's deps; names still refresh via the name-refresh effect.                                                                                                                  |
| L4  | LOW      | Duplication in the new module: `StoreGroupMessageInput` declared in both service + repository, and the service hand-mapped the doc → `StoredGroupMessage` instead of reusing the repo's `toStoredGroupMessage`.                                                                     | **Fixed** — input type now lives in the repository (imported by the service); `repository.create` returns the mapped `StoredGroupMessage`, which the service returns directly.                                                   |
| L5  | LOW      | ADR 0004 prose said scope/sort on `timestamp` + "no index change"; the code (correctly) uses server `createdAt` + a `{groupId, createdAt, _id}` index.                                                                                                                              | **Fixed** — ADR corrected to `createdAt` and the index note updated.                                                                                                                                                             |
| L6  | LOW      | `GroupMessage` maps to the Mongoose-default `groupmessages`, not snake_case `group_messages` (CLAUDE.md §4). Pre-existing project-wide pattern (`groupmembers`, `conversationmessages` too) — the new module is consistent with existing code.                                      | **Deferred → `refactoring`.** Project-wide collection-name normalization, not this PR.                                                                                                                                           |
