...
**All blocking issues resolved; N1/N2/N6 also landed.** The functional bugs (F1–F3, R1–R3) and the usability/error-handling notes (N1, N2, N6) are fixed with regression tests; `lint`/`typecheck` are clean and the full test suite is green (backend 330/330, frontend 187/187 at `--maxWorkers=2`; the two heavy screen suites time out only under unbounded parallel load — an unrelated, pre-existing test-runner flake). Deferred (documented, not blocking): N3 (load-more/pagination), N4 (shared envelope types), N5 (`ciphertext` field naming), N7 (recipient-exists relay guard). Ready for final security/code-review sign-off, then merge.
...
- [x] `pnpm lint` / `pnpm typecheck` green both workspaces; `pnpm test` green — backend 330/330, frontend 187/187 (the two heavy screen suites time out only under full-parallel load; green isolated and at `--maxWorkers=2`)
