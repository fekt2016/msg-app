...
Standardize the REST API conventions used by every backend endpoint so clients (mobile and web) can rely on consistent URLs, responses, errors, and query semantics.
...
4. **Validate at the edge, respond uniformly**: all input validated by Zod middleware; all errors shaped by the central error handler (see `ecommerce-backend-architecture.md`).
...
- `DELETE /api/v1/products/:id` — delete → `204` (or soft-delete per `ENGINEERING_RULES.md` §4)
...
- Query params: `page` (1-based, default 1), `pageSize` (default 20, max 100 — see `ENGINEERING_RULES.md` §3).
...
- Sorting via `sort=field` and `sort=-field` (desc); whitelist allowed sort keys per endpoint — see `ecommerce-product-catalog.md`.

### Versioning

- Path versioning `/api/v1/...`; a new breaking version increments the prefix (`/api/v2`) — never silently change field semantics.
- Deprecate old versions with a documented window; return deprecation header/notice, not silent breakage.

### Rate Limiting

- Apply per-IP and per-user where applicable: public endpoints, auth (stricter), OTP, uploads — tiers in `ENGINEERING_RULES.md` §6.
- `429` responses include `Retry-After` header and a standard rate-limit error body.
- Redis-backed limiter so limits hold across instances (see `ecommerce-backend-architecture.md`).
...
- Include `X-Request-Id` in responses and logs for debugging (see `ecommerce-backend-architecture.md` §Logging).
- Document endpoints with OpenAPI/Swagger per `ENGINEERING_RULES.md` §3.
...
- Cache list reads where data is infrequently written (see `ecommerce-product-catalog.md`).
...
- Mixing versions inside one controller.
...
- Changing field meaning without a version bump.
...
2. Implement the shared envelope + error handler once in the app bootstrap (`ecommerce-backend-architecture.md`).
3. Add the validate middleware to the route; keep the controller a thin HTTP mapper.
4. Document the endpoint in OpenAPI and add tests per `ecommerce-testing.md`.
5. Rate-limit appropriately per endpoint tier and update `TASKS.md`.
